...
class Baseline(object):
    def __init__(self, args):
        ...
        #######################################
        try:
            # Load the .pth file using torch.load
            pretrained_vgg = torch.load('./baseline_checkpoints/vgg_normalised_conv5_1.pth',
                                        map_location=torch.device('cpu'))  # Removed weights_only
        except Exception as e:
            print(f"Error loading pretrained VGG: {e}")
    ...
    def calc_style_loss(self, input, target, dim=False):
        ...
        if dim == True:
            return torch.mean(
                self.MSE_instance_loss(input_mean, target_mean) + self.MSE_instance_loss(input_std, target_std),
                dim=(1, 2))
    ...
    def patch_wise_cos(self, image, level, patch_size, stride, path):
        ...
        for idx in range(B):
            ...
            for patch in range(1, patches_num):
                gram_statistic.append(cos(gram_matrix(fix_patch.unsqueeze(0)),
                                          gram_matrix(patches[idx][patch].unsqueeze(0))).mean().item())
    ...
    def proposed_local_gram_loss_v2(self, stylization, style, alpha):
        ...
        for batch in range(B):
            ...
            for level in [4, 5]:
                stylization_patches = self.network.encoder.get_features(
                    self.extract_image_patches(stylization[batch:batch + 1], window_size, window_size).squeeze(0),
                    level)
                style_patches = self.network.encoder.get_features(
    ...
    def styleaware_regularizer(self, stylized_results, style):
        ...
        reg = 0
        for ratio in ratios:
            stylized_std = torch.nn.functional.log_softmax(
                self.local_global_gram_loss(stylized_results, level, ratio, ''))
    ...
    def contrastive_styleaware_regularizer(self, content, style, style_adaptive_alpha, gray_content, gray_style):
        ...
        for B in range(self.batch_size):
            ...
            style_adaptive_alpha_ = torch.vstack(
                [style_adaptive_alpha, style_adaptive_alpha[B].unsqueeze(0).repeat(self.batch_size - 1, 1)])
            ...
            stylized_results, _, _, _, _ = self.network(content_, style_, style_adaptive_alpha_,
                                                        self.invert_gray(content_), self.invert_gray(style_))
    ...
    def calc_adaptive_alpha(self, style, gray_style):
        color_style_alpha_8 = (((self.adaptive_gram_weight(style, 1, 8) + self.adaptive_gram_weight(style, 2,
                                                                                                    8) + self.adaptive_gram_weight(
            style, 3, 8)) / 3).unsqueeze(1).cuda())
        gray_style_alpha_8 = ((self.adaptive_gram_weight(gray_style, 1, 8) + self.adaptive_gram_weight(gray_style, 2,
                                                                                                       8) + self.adaptive_gram_weight(
            gray_style, 3, 8)) / 3).unsqueeze(1).cuda()

        color_style_alpha_16 = (((self.adaptive_gram_weight(style, 1, 16) + self.adaptive_gram_weight(style, 2,
                                                                                                      16) + self.adaptive_gram_weight(
            style, 3, 16)) / 3).unsqueeze(1).cuda())
        gray_style_alpha_16 = ((self.adaptive_gram_weight(gray_style, 1, 16) + self.adaptive_gram_weight(gray_style, 2,
                                                                                                         16) + self.adaptive_gram_weight(
            gray_style, 3, 16)) / 3).unsqueeze(1).cuda()
    ...
    def train(self):
        ...
        content_loader = torch.utils.data.DataLoader(content_set, batch_size=self.batch_size, shuffle=True,
                                                     drop_last=True, num_workers=self.num_workers)
        art_reference_loader = torch.utils.data.DataLoader(art_reference_set, batch_size=self.batch_size, shuffle=True,
                                                           drop_last=True, num_workers=self.num_workers)
        ...
        for iteration in range(self.max_iter):
            ...
            style_adaptive_alpha = (((self.adaptive_gram_weight(style, 1, 8) + self.adaptive_gram_weight(style, 2,
                                                                                                         8) + self.adaptive_gram_weight(
                style, 3, 8)) / 3).unsqueeze(1).cuda() + \
                                    ((self.adaptive_gram_weight(gray_style, 1, 8) + self.adaptive_gram_weight(
                                        gray_style, 2, 8) + self.adaptive_gram_weight(gray_style, 3, 8)) / 3).unsqueeze(
                                        1).cuda()) / 2

            content_recon, _, _, _, _ = self.network(gray_content, content, torch.ones_like(style_adaptive_alpha),
                                                     gray_content, content)
            ...
            stylization, attn_style_4_1, attn_style_5_1, attn_map_4_1, attn_map_5_1 = self.network(content, style,
            ...
            D_loss = self.bce_loss(origin_gan_output, ones_like(origin_gan_output)) + self.bce_loss(style_gan_output,
                                                                                                    zeros_like(
                                                                                                        style_gan_output))
            ...
            for level in [2, 3, 4, 5]:
                ...
                identity_loss2 += (self.calc_content_loss(recon_content_feat, content_feat) + self.calc_content_loss(
                    recon_style_feat, style_feat))
                ...
                if level in [4, 5]:
                    ...
                    if level == 4:
                        cx_loss += ((contextual_loss_v2(content_feat, recon_content_feat)) + (
            ...
            attnded_style_loss = torch.mean(
                (self.MSE_instance_loss(gram_matrix(attn_style_4_1),
                                        gram_matrix(self.network.encoder.get_features(stylization, 4))) + \
                 self.MSE_instance_loss(gram_matrix(attn_style_5_1),
                                        gram_matrix(self.network.encoder.get_features(stylization, 5)))), dim=(1, 2)
            )
            ...
            total_loss = 1.0 * cx_loss + 1.0 * content_loss + 1.0 * identity_loss2 + 0.2 * local_style_loss + 100.0 * torch.mean(
                attnded_style_loss * style_adaptive_alpha) + \
                         10.0 * torch.mean(global_style_loss * (
                        1 - style_adaptive_alpha)) + 1.5 * color_loss + tv_loss  # + 0.1*G_loss #Final_v9_adv_t2_no_adv
            ...
            if ((iteration) == 100) or ((iteration) % self.check_iter) == 0:
                print("%s: Iteration: [%d/%d]\tC_loss: %2.4f" % (
                time.ctime(), iteration, self.max_iter, total_loss.item()))
                wandb.log({"Recon_results": wandb.Image(
                    denorm(tensor=torch.cat([content, content_recon, style, style_recon, stylization]),
    ...
    def test(self, args):
        self.network.decoder.load_state_dict(
            torch.load(os.path.join(self.result_st_dir, 'dec_model_.pth'))['state_dict'])
        self.network.transformer.load_state_dict(
            torch.load(os.path.join(self.result_st_dir, 'transformer_model_.pth'))['state_dict'])

        content_set = Transfer_TestDataset(self.content_dir, (512, 512), self.cropsize, self.cencrop, type='art',
                                           is_test=True)
        art_reference_set = Transfer_TestDataset(self.style_dir, (512, 512), self.cropsize, self.cencrop, type='art',
                                                 is_test=True)

        dir_path = os.path.join(args.test_result_dir, self.comment, 'stylized_results')
        os.makedirs(dir_path, exist_ok=True)
        content_loader = torch.utils.data.DataLoader(content_set, batch_size=self.batch_size, shuffle=False,
                                                     drop_last=True, num_workers=self.num_workers)
        art_reference_loader = torch.utils.data.DataLoader(art_reference_set, batch_size=self.batch_size, shuffle=False,
                                                           drop_last=True, num_workers=self.num_workers)
        ...
        for iteration in range(1, (N // self.batch_size) + 1):
            ...
            imsave(stylization, os.path.join(dir_path, 'single_art_stylized_' + str(iteration) + '_' + str(
                style_adaptive_alpha.item()) + '.png'), nrow=self.batch_size)
    ...
    def content_fidelity(self, args):
        ...
        self.network.transformer.load_state_dict(
            torch.load(os.path.join(self.result_st_dir, 'transformer_model_.pth'))['state_dict'])
        content_set = Transfer_TestDataset(self.content_dir, (self.imsize, self.imsize), self.cropsize, self.cencrop,
                                           type='art', is_test=False)
        art_reference_set = Transfer_TestDataset(self.style_dir, (self.imsize, self.imsize), self.cropsize,
                                                 self.cencrop, type='art', is_test=False)
        ...
        for iteration in range(1, (N // self.batch_size) + 1):
            ...
            for l in [2, 3, 4, 5]:
                ...
                CE += cos(
                    self.network.encoder.get_features(torch.nn.functional.interpolate(stylization, size=(256, 256)), l),
                    self.network.encoder.get_features(torch.nn.functional.interpolate(content, size=(256, 256)),
                                                      l)).abs().mean().item()
            CE = CE / 4
            total_CE += CE
    ...
    def eval(self, args):
        ...
        self.network.transformer.load_state_dict(
            torch.load(os.path.join(self.result_st_dir, 'transformer_model_.pth'))['state_dict'])
        content_set = Transfer_TestDataset(self.content_dir, (self.imsize, self.imsize), self.cropsize, self.cencrop,
                                           type='art', is_test=True)
        art_reference_set = Transfer_TestDataset(self.style_dir, (self.imsize, self.imsize), self.cropsize,
                                                 self.cencrop, type='art', is_test=True)
        content_loader = torch.utils.data.DataLoader(content_set, batch_size=self.batch_size, shuffle=False,
                                                     drop_last=True, num_workers=self.num_workers)
        art_reference_loader = torch.utils.data.DataLoader(art_reference_set, batch_size=self.batch_size, shuffle=False,
                                                           drop_last=True, num_workers=self.num_workers)
        ...
        for iteration in range(1, (N // self.batch_size) + 1):
            ...
            stylized_adaptive_alpha = (((self.adaptive_gram_weight(stylized_results, 1, 8) + self.adaptive_gram_weight(
                stylized_results, 2, 8) + self.adaptive_gram_weight(stylized_results, 3, 8)) / 3).unsqueeze(1).cuda() + \
                                       ((self.adaptive_gram_weight(gray_stylized, 1, 8) + self.adaptive_gram_weight(
                                           gray_stylized, 2, 8) + self.adaptive_gram_weight(gray_stylized, 3,
                                                                                            8)) / 3).unsqueeze(
                                           1).cuda()) / 2
